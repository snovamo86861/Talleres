...
import centralpacientes.mundo.ListaPacientes;
...
public class InterfazCentralPacientes extends JFrame {
     ...
     */
    private ListaPacientes central;
    ...
    public InterfazCentralPacientes() {
        ...
        central = new ListaPacientes();

        // Llena la lista de clínicas
    ...
    public void mostrarDialogoOpcionesAgregarPaciente() {
        if (central.darNumeroPacientes() > 0) {
            ...
            mostrarDialogoInsertarPaciente(FINAL, -1);
        }
    }
    ...
    public void agregarPaciente(int formaAdicion, int codPaciente, int elCodigo, String elNombre, String laClinica, String laHistoriaClinica, int elSexo) throws NoExisteException, YaExisteException {
        Paciente paciente = new Paciente(elCodigo, elNombre, laClinica, elSexo, laHistoriaClinica);
    ...
    public void refrescarListaPacientes() {
        ...
        for (int i = 0; i < central.darNumeroPacientes(); ++i) {
            Paciente p = central.obtenerPaciente(i);
            pacs.add(p);
        }

        panelLista.refrescarLista(pacs);
    }
     ...
     */
    public void mostrarNombreMasLargo() {
        if (central.darNumeroPacientes() > 0) {
            Paciente pac = central.nombreMasLargo();
            JOptionPane.showMessageDialog(this,
                    "El paciente con el nombre más largo\n" + pac.toString(),
                    "Respuesta", JOptionPane.INFORMATION_MESSAGE);
        }
        else {
            JOptionPane.showMessageDialog(this, "ERROR: No hay pacientes",
    ...
    public void porcentaje() {
        if (central.darNumeroPacientes() == 0) {
            ...
            Object[] clinicas = listaClinicas.toArray();
            String n = (String) JOptionPane.showInputDialog(this, "Nombre de la clínica",
                    "Porcentaje", JOptionPane.QUESTION_MESSAGE, null, clinicas, clinicas[0]);
            if (n != null) {
                double porc = central.porcentajeClinica(n);
                String res = String.format("El porcentaje de pacientes en la %s es %.2f %%",
                        n, porc);
                JOptionPane.showMessageDialog(this,  res, "Porcentaje", JOptionPane.INFORMATION_MESSAGE);
            }
